...
export async function POST(context) {
  ...
  try {
    ...
    console.log(data); // <-- conservamos el log que tenías
    ...
    // ----> NO HACEMOS auto-login aquí (se removió authWithPassword y session.set)

    // Conservamos el log para ver en la terminal los datos guardados
    console.log('Usuario creado:', createdUser);

    // 9. Devuelve una respuesta de éxito con los datos del usuario (sin iniciar sesión)
    return new Response(JSON.stringify({
      status: 'success',
      message: 'Usuario registrado exitosamente.',
      user: createdUser, // <-- aquí devolvemos createdUser (en lugar de authData.record)
    }), {
